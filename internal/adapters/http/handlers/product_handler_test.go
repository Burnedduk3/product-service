...
func TestProductHandler_UpdateProductStock_InvalidStock(t *testing.T) {
	// Setup
	handler, mockUseCases := setupTestHandler()
	...
	mockUseCases.On("UpdateProductStock", mock.Anything, uint(1), -10).Return(nil, domainErrors.ErrInvalidProductStock)
	...
	assert.Equal(t, "INVALID_STOCK", response.Error)
	mockUseCases.AssertExpectations(t)
}

func TestProductHandler_UpdateProductPrice_InvalidPrice(t *testing.T) {
	// Setup
	handler, mockUseCases := setupTestHandler()
	...
	mockUseCases.On("UpdateProductPrice", mock.Anything, uint(1), -100.0).Return(nil, domainErrors.ErrInvalidProductPrice)
	...
	assert.Equal(t, "INVALID_PRICE", response.Error)
	mockUseCases.AssertExpectations(t)
}
